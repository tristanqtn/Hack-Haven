...
def decrypt(encrypted):
    ...
    # Create a pool of workers to parallelize the decryption attempts
    with Pool() as pool:
        for num_rounds in range(1, 7):
            # Generate possible combinations of A and B
            possible_values = itertools.product(range(2, n), repeat=6 * num_rounds)
            args = ((num_rounds, A, B) for A in possible_values for B in possible_values)
            pool.map(attempt_decrypt, args)
